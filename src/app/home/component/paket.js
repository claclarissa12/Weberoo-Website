...
export default function Paket() {
  ...
  return (
    <div id="price" className="bg-gray-50 px-4 md:px-10 py-14 text-center">
      {/* Judul */}
      <motion.h2 className="text-3xl font-bold text-center mb-12 text-[#112D4E]">
      ...
      <motion.h3 className="bg-[#3F72AF] inline-block text-white px-5 py-1.5 rounded-lg font-semibold text-sm md:text-base mb-5">
        HARGA
      </motion.h3>

      <motion.p className="text-base md:text-lg mb-10 text-gray-700">
      ...
      >
        {packages.map((paket, i) => (
          ...
          >
           ...
           <button
                onClick={() => router.push(`/home/form-order?paket=${paket.name}`)}
                className="bg-[#3F72AF] text-white text-sm px-3 py-1.5 rounded-lg hover:bg-[#2c5480] transition-all duration-300"
                >
                Beli Sekarang
            </button>
